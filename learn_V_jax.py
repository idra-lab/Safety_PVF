...
with tqdm(range(epochs), desc="Epoch") as pbar:
    for epoch in pbar:
        ...
        if epoch % 100 == 0:
            V_net = model.apply(state.params, state_grid)

            plt.figure(figsize=(10, 5))
            plt.grid(True)
            plt.gca().set_aspect('equal')
            plt.plot(state_grid,
                    V_prob, 'blue', label="V_prob")
            ...
            plt.savefig(
                f'plots/V_comparison_x_th:{env_settings.x_th}_u_max:_{env_settings.u_max}_d_max:_{env_settings.d_max}_distr_{env_settings.distr}.png')
            # plt.show()
            plt.close()
