...
if not(os.path.exists(f'data_pairs_state_size{state_size}.npy')):
    data_pairs_chunk = []
    ...
    np.save(f'data_pairs_state_size{state_size}.npy',data_pairs)
else: data_pairs = np.load(f'data_pairs_state_size{state_size}.npy')
...
nn_V_safe = NeuralNetwork(state_size, 256, 1, 3,log_representation=dyn_settings.log_repr,mean=mean,std=std).to(device)
nn_target = NeuralNetwork(state_size, 256, 1, 3).to(device)
