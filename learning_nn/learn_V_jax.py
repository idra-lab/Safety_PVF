...
from system_dyn import DynSystem
...
#            Load Dataset
# ====================================
def generate_pairs(data,input_size):
    """
    ...
    for i in tqdm(range(data.shape[0])):
        for j in range(data.shape[1] - 1):
            if (data[i, j, :] != np.zeros(data.shape[2])).any() and (
                ...
                pairs.append(np.hstack((data[i, j, 0], data[i, j + 1, :])))
    return np.array(pairs)


data_path = 'training_data_interrupted_n1.npy'
data = np.load(data_path)
# data = data[:10000]
state_grid = np.load('grid.npy')
state_grid = state_grid.reshape((state_grid.shape[0],1))
V_prob = np.load('V_prob_x_th:-1.5_u_max:_1_d_max:_4_distr_gaussian.npy')
print("Dataset loaded:", data.shape)
env_settings = DynSystem()

input_dim = 1

# # data = generate_pairs(data,input_dim)

# states = data[ :, 0]
# next_states = data[ :, input_dim]
# dones = data[ :, -1]
# capt_p = data[ :, -2]

states = data[:, 0::2, :input_dim]
next_states = data[:, 1::2, :input_dim]
dones = data[:, 1::2, -1:]
capt_p = data[:, 1::2, -2:-1]
...
def get_batches(states, next_states, dones, capt_p, batch_size, rng):
    ...
    batches = jax.vmap(extract)(indices)      # apply vectorized version of extract
    return batches, rng

def plot_training(ep):
    V_net = model.apply(
            state.params,
            jnp.array((np.tile(state_grid,input_dim) - mean)/(std-1e-8),dtype=jnp.float32)
    )

    print(
        f"output for 1.9: {model.apply(state.params,jnp.array((np.tile(np.array([1.9]),input_dim) - mean)/(std-1e-8),dtype=jnp.float32))}"
    )
    ...
    plt.legend()
    plt.title(
        f"V(x)  x_th: {env_settings.x_th} u_max: {env_settings.u_max} d_max: {env_settings.d_max} noise: {env_settings.distr}"
    )

    plt.savefig(
        f"plots/V_comparison_x_th:{env_settings.x_th}_u_max:_{env_settings.u_max}_d_max:_{env_settings.d_max}_distr_{env_settings.distr}_log_repr{env_settings.log_repr}_ep{ep}_JAX.png"
    )
    ...
    plt.legend()
    plt.title(
        f"V(x)  x_th: {env_settings.x_th} u_max: {env_settings.u_max} d_max: {env_settings.d_max} noise: {env_settings.distr} log_10(1-V(x))"
    )
    plt.savefig(
        f"plots/V_comparison_x_th:{env_settings.x_th}_u_max:_{env_settings.u_max}_d_max:_{env_settings.d_max}_distr_{env_settings.distr}_log_repr_ep{ep}_JAX.png"
    )
    # plt.show()
    plt.close()
